...
// CLAUDE.md準拠: XSS対策・暗号乱数・チップは小数1桁まで (0.1単位)
...
// プレイヤー0の役判定表示キャッシュ
// 一度表示された以降はアニメーション中も直前の判定を表示し続ける (ちらつき防止)
// 新しいハンド開始時 / UI初期化時にクリア
let _cachedLiveHand = '';
...
export function initUI(gameAdapter, config, decideCpu, callbacks = {}) {
  ...
  // Fixed Limit / ホールカード枚数 / プレイヤー数に応じて data 属性を設定（CSS で条件分岐）
...
function _startStudDoorReveal() {
  ...
  for (let i = 0; i < active.length; i++) {
    setTimeout(() => {
      ...
      // 最後の1枚リビール後 → アクション開始
      // 650ms: stud-float アニメーション (0.55s) + 緩衝 100ms
...
function _startStudStreetDeal() {
  ...
  for (let i = 0; i < active.length; i++) {
    setTimeout(() => {
      ...
      // 最後の1枚ディール後 → アクション開始
      // 650ms: stud-float アニメーション (0.55s) 完了まで待機し、役判定表示もここで解禁
      if (i === active.length - 1) {
        setTimeout(() => {
...
function _renderStudCards(p, playerIdx, isHuman, isSD) {
  ...
  // フォールド済みプレイヤーの表示:
  // - 3rd Street (hand.length <= 3): ドアカード(アップカード)を透過表示、ダウンカードは裏向き
  // - 4th Street以降: カード非表示（プレースホルダーのみ）
  // - ショーダウン/完了 (isSD): ダウンカードも含めて公開 (相手のフォールドハンド開示)
  if (p.folded) {
    const isThirdStreet = p.hand.length <= 3;
    const revealAll     = isHuman || isSD; // isSD = SHOWDOWN/COMPLETE
    let upHtml = '';
    for (let i = 0; i < 4; i++) {
      if ((isThirdStreet || revealAll) && i < upCards.length) {
    ...
    let downHtml = '';
    for (let i = 0; i < 3; i++) {
      if (revealAll && i < downCards.length) {
        // ショーダウン/完了: ダウンカードも表向きで公開
        downHtml += cardHtml(downCards[i], 'hole stud-down');
      } else if (isThirdStreet && i < downCards.length) {
        // 3rd Street 途中: 裏向き
...
// ------- Fixed Limit ベットサイズ計算 -------
function _getStudFixedBetSize() {
  // Stud (Fifth以降), Draw (BETTING_3/4), FLO8 (TURN/RIVER) — 全て bigBet
  // adapter 側の _roundBetSize がある場合はそれを優先 (ゲーム側の厳密な判定)
  if (adapter && typeof adapter._roundBetSize === 'number') {
    return adapter._roundBetSize;
  }
  const bigBetStates = [
    'FIFTH_STREET', 'SIXTH_STREET', 'SEVENTH_STREET',  // Stud 系
    'BETTING_3', 'BETTING_4',                           // Draw 系 (27TD/Badugi)
    'TURN', 'RIVER',                                    // FLO8 (NLH風ストリート名)
  ];
...
function computeRaisePresets(player) {
  ...
  // 通常の上限: 全チップ (オールイン)
  let maxTotal = isRaise ? player.chips + player.currentBet : player.chips;
  // Pot Limit 制約: ベット/レイズの最大総額を「コール後のポットサイズ」にクランプ
  //   BET 時: 新 currentBet = pot
  //   RAISE 時: 新 currentBet = adapter.currentBet + (pot + toCall)
  if (gameConfig?.bettingStructure === 'pot-limit') {
    const potLimitTotal = isRaise
      ? adapter.currentBet + adapter.pot + toCall
      : adapter.pot;
    maxTotal = Math.min(maxTotal, potLimitTotal);
  }
...
function renderWinOverlay() {
  ...
  const hiW = adapter.lastHiWinners;
  const loW = adapter.lastLoWinners;
  const isHiLoGame = !!gameConfig.hasLowHand && Array.isArray(hiW) && Array.isArray(loW);

  // ── Stud 8 (Hi/Lo) 専用表示 ──
  if (isHiLoGame && hiW.length > 0) {
    const scoop = hiW.length === 1 && loW.length === 1 && hiW[0].id === loW[0].id;

    if (scoop) {
      // スクープ: 1人が Hi も Lo も勝利
      const w = hiW[0];
      document.getElementById('win-player-name').textContent = `${w.name.toUpperCase()} SCOOPS`;
      document.getElementById('win-hand-name').innerHTML =
        _renderStud8HandBlock(w.hiResult, w.loResult);
    } else if (loW.length === 0) {
      // Lo 不成立 → Hi 総取り (Hi 表記)
      const names = hiW.map(w => w.name.toUpperCase()).join(' & ');
      document.getElementById('win-player-name').textContent = names;
      document.getElementById('win-hand-name').innerHTML =
        _renderStud8HandBlock(hiW[0].hiResult, null);
    } else {
      // 通常スプリット: Hi 勝者と Low 勝者の両方を 1 ブロックに表記
      // Hi 勝者表示: 役名を "{name(s)} — ワンペア" のように結合、Low 勝者も同様
      const hiNames = hiW.map(w => w.name).join(' & ');
      const loNames = loW.map(w => w.name).join(' & ');
      const hiName  = hiW[0].hiResult?.name || '';
      document.getElementById('win-player-name').textContent = 'HI / LOW SPLIT';
      // 合成用: Hi 役名を {names} — {name} に置換した疑似 Hi 結果
      const hiFused = hiW[0].hiResult ? { ...hiW[0].hiResult, name: `${hiNames} — ${hiName}` } : null;
      const loFused = loW[0].loResult
        ? { ...loW[0].loResult, name: (() => {
            const orig = loW[0].loResult.name;
            // "7-Low (7-5-4-3-2)" のキッカー部を保持したまま主部に名前を差し込む
            const m = orig.match(/^(.*?)\s*(\([^)]*\))\s*$/);
            if (m) return `${loNames} — ${m[1]} ${m[2]}`;
            return `${loNames} — ${orig}`;
          })() }
        : null;
      document.getElementById('win-hand-name').innerHTML =
        _renderStud8HandBlock(hiFused, loFused);
    }
    // ポット表示:
    //   スクープ → 総額
    //   Hi総取り (Lo不成立):
    //     1勝者 → 総額 / 複数タイ → 1人あたり表示 (×N)
    //   Hi/Lo スプリット → Hi/Lo それぞれの1人あたり獲得額 (タイならクォーター=1/4pot)
    const isSplit = !scoop && loW.length > 0;
    const hiAmt = adapter.lastHiAmount ?? adapter.lastPot;
    const loAmt = adapter.lastLoAmount ?? 0;
    const winPotEl = document.getElementById('win-pot');

    if (isSplit) {
      // Hi/Lo 分割: 1人あたりの獲得額 (winners.length === 1 → half, === 2 → quarter)
      const hiCount = hiW.length;
      const loCount = loW.length;
      const hiPer   = Math.floor(hiAmt / hiCount);
      const loPer   = Math.floor(loAmt / loCount);
      const hiStr   = `Hi + ${bb(hiPer, BB)} BB${hiCount > 1 ? ` ×${hiCount}` : ''}`;
      const loStr   = `Lo + ${bb(loPer, BB)} BB${loCount > 1 ? ` ×${loCount}` : ''}`;
      winPotEl.textContent = `${hiStr}  /  ${loStr}`;
    } else if (!scoop && hiW.length > 1) {
      // Hi 総取りだが複数タイ: 1人あたり表示
      const hiPer = Math.floor(adapter.lastPot / hiW.length);
      winPotEl.textContent = `+ ${bb(hiPer, BB)} BB ×${hiW.length}`;
    } else {
      // 単独勝者 (scoop / Hi 単独総取り)
      winPotEl.textContent = `+ ${bb(adapter.lastPot, BB)} BB`;
    }
    winPotEl.classList.toggle('win-pot-split', isSplit);
  ...
  const winPotEl2 = document.getElementById('win-pot');
  winPotEl2.textContent = `+ ${bb(adapter.lastPot, BB)} BB`;
  winPotEl2.classList.remove('win-pot-split');  // Stud8 分割クラス残留を解除
  overlay.classList.remove('hidden');
}

// ------- actionLog 走査: 指定プレイヤーが現ハンド内で 1 度でもアクションしたか -------
// (ハンド開始時に actionLog が空にリセットされるため、log 全体をスキャンすれば十分)
// resetHasActed で player.hasActed が false に戻されても、log には痕跡が残るのでこちらを使う
function _hasActedInCurrentHand(adapter, playerName) {
  ...
  const prefixes = [
    ...
    `${playerName}  calls`,
    `${playerName}  bets`,
    `${playerName}  raises to`,
    `${playerName}  completes to`,
    `${playerName}  ALL IN`,
  ];
...
// ------- Stud 8 用: Hi / Low 役の3行HTMLブロック（コロン揃え、キッカーは別行） -------
//   Hi : {hiName}
//   Low: {loMain}
//        (loKickers)         ← Low が成立時のみ、値列にインデント
//   Low が不成立 → "Low: None" 行のみ
function _renderStud8HandBlock(hiResult, loResult) {
  const hiName = hiResult && hiResult.name ? hiResult.name : '';
  if (!hiName) return '';

  // Low 名を "主部" と "(キッカー)" に分離
  // "7-Low (7-5-4-3-2)" → main="7-Low" / kick="(7-5-4-3-2)"
  // "Wheel (A-5)"       → main="Wheel" / kick="(A-5)"
  let loMain = 'None';
  let loKick = '';
  if (loResult && loResult.name) {
    const m = loResult.name.match(/^(.*?)\s*(\([^)]*\))\s*$/);
    if (m) { loMain = m[1]; loKick = m[2]; }
    else   { loMain = loResult.name; }
  }

  let html = `<div class="s8-hand">`
    + `<span class="lbl">Hi :</span><span class="val">${esc(hiName)}</span>`
    + `<span class="lbl">Low:</span><span class="val">${esc(loMain)}</span>`;
  if (loKick) {
    html += `<span class="kickers">${esc(loKick)}</span>`;
  }
  html += `</div>`;
  return html;
}
...
function renderPlayers() {
  ...
  adapter.players.forEach((p, i) => {
    ...
    // SB/BB チップ (ブラインド制ゲームのみ / Stud系アンテ制は非表示)
    //   "最初のベッティングラウンド" かつ SB/BB プレイヤーが "まだ一度もアクションしていない"
    //   間は "SB  0.5 BB" / "BB  1 BB" と金額併記。
    //   一度でもアクション (FOLD/CHECK/CALL/BET/RAISE/ALL_IN) すれば "SB" / "BB" に戻り、
    //   後から他プレイヤーにレイズされ再度ターンが回ってきても金額表示は復活しない。
    //   hasActed は resetHasActed で false に戻るため使えず、actionLog で永続判定する。
    //
    //   各ゲームの初回ベッティングラウンド state:
    //     NLH / LH (FLH) / FLO8: 'PREFLOP'
    //     2-7 Triple Draw / Badugi: 'BETTING_1'
    //     Stud/Razz/Stud 8: 非該当 (isStud フィルタで除外)
    const hasBlinds = !!gameConfig.hasBlinds;
    const sbBBAmt   = (gameConfig.smallBlind && gameConfig.bigBlind)
      ? bb(gameConfig.smallBlind, gameConfig.bigBlind) : '0.5';
    const bbBBAmt   = gameConfig.bigBlind ? bb(gameConfig.bigBlind, gameConfig.bigBlind) : '1';
    const isFirstBettingRound = adapter.state === 'PREFLOP' || adapter.state === 'BETTING_1';
    const actedInHand = _hasActedInCurrentHand(adapter, p.name);
    const showSbAmt = hasBlinds && isSB && isFirstBettingRound && !actedInHand;
    const showBbAmt = hasBlinds && isBB && isFirstBettingRound && !actedInHand;
    const posChip = isStud ? ''
                  : isSB ? `<div class="pos-chip bsb">${showSbAmt ? `SB&nbsp;&nbsp;${sbBBAmt}&nbsp;BB` : 'SB'}</div>`
                  : isBB ? `<div class="pos-chip bbb">${showBbAmt ? `BB&nbsp;&nbsp;${bbBBAmt}&nbsp;BB` : 'BB'}</div>`
                  : '';
    ...
    if (isStud && p.hand.length) {
      ...
      if (p.folded) {
        // ショーダウン/完了時は相手のフォールドハンドも公開 (isSD)
        cards = (isHuman || isSD)
    ...
    const isStud8 = !!gameConfig.hasLowHand;

    // Stud 8 ショーダウン: Hi / Low の2行表示
    let hname = '';
    if (isSD && p.handResult && !p.folded) {
      if (isStud8) {
        hname = _renderStud8HandBlock(p.hiResult, p.loResult);
      } else {
        hname = `<span class="hname">${esc(p.handResult.name)}</span>`;
      }
    }
    ...
    // 生役表示（プレイヤー0自身 / ショーダウン以外）
    // Stud系: カード配布アニメーション中 (_studDoorRevealing/_studStreetDealing) は
    //        新規評価を一時停止し、直前のキャッシュ (_cachedLiveHand) を継続表示する。
    //        キャッシュが空 (初回評価前) はアニメ完了まで非表示。
    //        一度表示されて以降は常に何かしらの判定が表示される (ちらつき防止)。
    // Hi/Lo系 (Stud 8 / FLO8): フォールド後も自身の役判定を表示し続ける (学習支援)
    // それ以外のゲーム: フォールドしたら非表示 (従来挙動)
    let liveHand = '';
    const studDealing = gameConfig?.isStudGame && (_studDoorRevealing || _studStreetDealing);
    const allowFoldedDisplay = !!gameConfig?.hasLowHand;  // Stud 8 / FLO8
    const canShowLive = i === 0 && !isSD && (!p.folded || allowFoldedDisplay);
    if (canShowLive) {
      if (studDealing) {
        // アニメ中: キャッシュがあればそれを表示
        liveHand = _cachedLiveHand;
      } else if (adapter.evaluateCurrentHand) {
        try {
          if (isStud8 && adapter.evaluateCurrentLo) {
            // Stud 8: Hi / Low 両方をリアルタイム表示
            const hi = adapter.evaluateCurrentHand(0);
            const lo = adapter.evaluateCurrentLo(0);
            if (hi && hi.name && hi.name !== '(invalid)') {
              liveHand = _renderStud8HandBlock(hi, lo);
            }
          } else {
          ...
          // 新しい評価が得られたらキャッシュ更新 (空文字は更新しない = 最後の表示を保持)
          if (liveHand) _cachedLiveHand = liveHand;
          else liveHand = _cachedLiveHand; // 評価不可 (< 4/5枚) はキャッシュ継続
        } catch(e) {
          // 評価エラー: キャッシュ継続
          liveHand = _cachedLiveHand;
        }
    ...
    // 所持BB数直下のベット額表示 (.pcbet) と卓上ベットチップ (.table-bet-chip) は削除。
    // ベット情報は action-bubble (ポップ表示) で十分認識できるため重複表示を廃止。
    seat.innerHTML = `
      ${arrow}
      ${cardsWrap}
      ${bubble}
    ...
    // .table-bet-chip (卓上ベットチップ) は廃止 — action-bubble で代替表示
  });
...
function renderActions() {
  ...
  if (valid.includes('check')) {
    ...
    // CALL ボタンは "追加で支払う額" のみ表示 (既に投入済みの額を除外)
    //   例: 自分が 1BB 投入済みで相手 3.5BB にレイズ → CALL ボタン表記は "CALL 2.5BB"
    const toCallChips = Math.max(0, adapter.currentBet - p.currentBet);
    const b = mkBtn('call', `<span class="btn-label">CALL</span><span class="btn-sub">${bb(toCallChips, BB)} BB</span>`);
    b.onclick = () => humanAction('call');
    leftGrp.appendChild(b);
  }
  bar.appendChild(leftGrp);

  // 中央グループ: レイズプリセット（NLH）、Fixedベット（Fixed Limit）、Complete（Stud）
  ...
  if (hasRaise || hasComplete) {
    if (gameConfig.bettingStructure === 'fixed-limit') {
      // Fixed Limit: 固定額ボタン（Stud Complete 含む）
      bar.appendChild(mkDivider());
      const midGrp = mkGroup();
...
function showSlider(act) {
  ...
  let   maxTotal = isRaiseA ? p.chips + p.currentBet : p.chips;
  // Pot Limit クランプ (PLO 等)
  if (gameConfig?.bettingStructure === 'pot-limit') {
    const toCall = adapter.currentBet - p.currentBet;
    const potLimitTotal = isRaiseA
      ? adapter.currentBet + adapter.pot + toCall
      : adapter.pot;
    maxTotal = Math.min(maxTotal, potLimitTotal);
  }
  const pot = adapter.pot;
  const bet = adapter.currentBet;
  // Pot Limit 時は max preset のラベルを "Pot" とする (maxTotal=pot 上限 ≠ 全チップ)
  const fullStackTotal = p.chips + p.currentBet;
  const maxLabel = (maxTotal < fullStackTotal) ? 'Pot' : 'All In';
  const presets = [
    { label:'Min',    total:minTotal },
    { label:'33%',    total:potSizeTotal(0.33, bet, pot) },
    { label:'50%',    total:potSizeTotal(0.5,  bet, pot) },
    { label:'100%',   total:potSizeTotal(1.0,  bet, pot) },
    { label:maxLabel, total:maxTotal },
  ].filter((ps,i,arr)=>{
...
function renderMobileActions() {
  ...
  // モバイル Custom ベット UI 展開中はボトムパネルを拡張するフラグ
  //   CSS 側の ".mobile-custom-open" 対応ルールで height + action-log bottom を調整
  const gameScreen = document.getElementById('game-screen');
  if (gameScreen) {
    gameScreen.classList.toggle('mobile-custom-open', !!mobileCustomMode);
  }
  ...
  let   maxTotal = toCall > 0 ? p.chips + p.currentBet : p.chips;
  // Pot Limit クランプ (PLO 等)
  if (gameConfig?.bettingStructure === 'pot-limit') {
    const potLimitTotal = toCall > 0
      ? adapter.currentBet + adapter.pot + toCall
      : adapter.pot;
    maxTotal = Math.min(maxTotal, potLimitTotal);
  }
  ...
  // プリセット行（NLH / Fixed Limit で分岐）
  if (hasRaise || hasComplete) {
    if (gameConfig.bettingStructure === 'fixed-limit') {
      // Fixed Limit: プリセット不要（固定額のみ）
    } else {
      // No Limit: プリセット + カスタム
      const pot  = adapter.pot;
      const bet  = adapter.currentBet;
      ...
      if (mobileCustomMode) {
        ...
        inp.oninput = () => {
          ...
          if (Number.isFinite(v) && v >= minTotal && v <= maxTotal) {
            ...
            if (rBtn) {
              // Pot Limit 対応: maxTotal が pot 上限に clamp されている場合は
              // MAX 到達 ≠ ALL IN。実際のスタック全額を超えた時のみ ALL IN。
              const fullStack = p.chips + p.currentBet;
              const isAI = v >= fullStack;
  ...
  if (valid.includes('call')) {
    ...
    callBtn.innerHTML = `<span class="mbl-act">CALL</span><span class="mbl-amt">${bb(toCallChips, BB)} BB</span>`;
  ...
  if (hasComplete && !hasRaise) {
    ...
    if (gameConfig.bettingStructure === 'fixed-limit') {
      ...
      // Pot Limit 対応: maxTotal が pot 上限 (stack 未満) に clamp されている場合は
      // スライダー MAX ≠ ALL IN。実際にスタック全額を投入する時のみ ALL IN に切り替える
      const fullStackTotal = p.chips + p.currentBet;
      const isAllIn = mobileRaiseAmount >= fullStackTotal;
...
function humanAction(act, amount = 0) {
  ...
  // Custom ベット UI を閉じる (次ターンまでパネル拡張を解除)
  mobileCustomMode = false;
...
function finishHandFast() {
  ...
  if (adapter.state !== 'COMPLETE' && adapter.state !== 'SHOWDOWN') {
    renderAll();
    // NLH: 全員オールインでアクション不可になった場合はランアウトへ
    if (isRunout()) { scheduleRunout(); return; }
...
function cpuTurn() {
  ...
  // NLH: 全員オールインで誰も行動できない → ランアウトを開始
  // (currentPlayerIndex === -1 のまま cpuTurn が scheduled されるケースを救済)
  if (isRunout()) { scheduleRunout(); return; }
