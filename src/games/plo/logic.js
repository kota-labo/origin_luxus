// Pot Limit Omaha (PLO) — ゲームエンジン
// CLAUDE.md準拠: チップは小数1桁まで (0.1単位)、状態遷移は厳密管理
// GameAdapter インターフェース準拠
//
// 構造:
//   - NLH のステートマシン / NL ベッティング / NLH 勝者決定ロジックを踏襲
//   - FLO8 の `evaluateHiOmaha` (Omaha 2+3 Hi 評価) を import で流用
//   - Pot Limit 独自制約: BET/RAISE の最大額を「ポットサイズまで」に clamp

import { createDeck, shuffleDeck } from '../../core/card.js';
import { evaluateHiOmaha } from '../flo8/evaluator.js';
import { determineWinners } from '../nlh/evaluator.js';
...
export class PLOGame {
  ...
  // ══════════════════════════════════════════════
  // Pot Limit ベット上限計算
  //   BET:   max = pot (現在のポットサイズまで)
  //   RAISE: max raise-above-call = pot + toCall (= pot after my call)
  //          → amount 引数 (raise 増分) の最大値
  //          → maxRaiseTotal = currentBet + pot + toCall
  // ══════════════════════════════════════════════
  potLimitMaxBetAmount() {
    return this.pot;
  }

  potLimitMaxRaiseAmount(player) {
    const toCall = this.currentBet - player.currentBet;
    return this.pot + toCall;
  }

  /** UI/CPU 用: Pot Limit 上限額 (amount 引数の最大値) */
  getPotLimitMaxAmount(playerId, isRaise) {
    const player = this.players[playerId];
    if (!player) return 0;
    return isRaise ? this.potLimitMaxRaiseAmount(player) : this.potLimitMaxBetAmount();
  }
  ...
  getValidActions(player) {
    ...
    if (player.chips > toCall) {
      ...
      // Pot Limit: ALL_IN は "スタック ≤ pot 上限" のショートスタック時のみ許可
      //   通常サイズのスタックでは RAISE/BET が pot 上限まで可能なので ALL_IN は不要
      const fullStackTotal = player.chips + player.currentBet;
      const potLimitTotal  = toCall === 0
        ? this.pot
        : this.currentBet + this.pot + toCall;
      if (fullStackTotal <= potLimitTotal) {
  ...
  performAction(playerIndex, action, amount = 0) {
    ...
    switch (action) {
      ...
      case Action.BET: {
        // Pot Limit: BET 額を max = pot にクランプ
        const potLimitMax = this.potLimitMaxBetAmount();
        const betAmount = Math.min(
          Math.max(this.bigBlind, Math.floor(amount)),
          Math.min(potLimitMax, player.chips)  // pot cap & all-in cap
        );
      ...
      case Action.RAISE: {
        ...
        // Pot Limit: raise 増分 (toCall を除いた部分) を max = pot+toCall にクランプ
        const potLimitRaiseMax = this.potLimitMaxRaiseAmount(player);
        const requestedAbove = Math.floor(amount);
        const cappedAbove    = Math.min(requestedAbove, potLimitRaiseMax);
        const raiseTotal     = Math.max(minRaiseChips, cappedAbove + toCall);
        // さらに全チップ以下に制限 (オールイン超過禁止)
        const finalRaiseTotal = Math.min(raiseTotal, player.chips);
        this.placeBet(player, finalRaiseTotal);
  ...
  // ══════════════════════════════════════════════
  // Showdown — Omaha Hi 単独評価 (NLH と同じ単一勝者ロジック)
  // ══════════════════════════════════════════════
  goToShowdown() {
    this.transition(GameState.SHOWDOWN);
    ...
    for (const p of contenders) {
      // Omaha Hi: 必ず手札2 + ボード3 の 60 通りから最強を選ぶ
      p.handResult = evaluateHiOmaha(p.hand, this.communityCards);
    }
