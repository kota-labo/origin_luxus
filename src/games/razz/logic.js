// Razz (Seven Card Stud A-5 Lowball) — ゲームエンジン
...
export class RazzGame {
  constructor(playerNames, config) {
    this.config     = config;
    this.smallBlind = config.smallBlind || 10;  // GameAdapter互換
  ...
  startHand() {
    ...
    // Bring-in プレイヤー特定（Razz: 最も高いドアカード = 最悪のロウ）
  ...
  getValidActions(playerOrId) {
    ...
    // Third Street: Bring-in 未投稿 → 最悪ドアカードプレイヤーが BRING_IN or COMPLETE を選択
  ...
  _advanceStreet() {
    ...
    // 行動順決定: Fourth Street+ は最良ロウ可視ハンドのプレイヤーから
    const active = this.activeInHandPlayers;
  ...
  /** UI用: 現在の役をリアルタイム評価（Razz A-5 Lowball） */
